...
FESTIVOS_BARCELONA_2025 = {
    # Festivos nacionales y catalanes
    (2025, 1, 1): "Año Nuevo",
    (2025, 1, 6): "Reyes",
    (2025, 4, 18): "Viernes Santo",
    (2025, 4, 21): "Lunes de Pascua",
    (2025, 5, 1): "Día del Trabajo",
    (2025, 6, 24): "San Juan",  # Festivo en Catalunya
    (2025, 8, 15): "Asunción",
    (2025, 9, 11): "Diada Nacional de Catalunya",
    (2025, 9, 24): "La Mercè",  # Festivo local Barcelona
    ...
    (2025, 12, 25): "Navidad",
    (2025, 12, 26): "San Esteban",  # Festivo en Catalunya
}

def is_festivo(date_obj):
    """Determina si una fecha es festivo en Barcelona"""
    year = date_obj.year
    ...
    if year == 2025:
        ...
        # Para otros años, no consideramos festivos específicos
        return False

def get_festivo_name(date_obj):
    """Obtiene el nombre del festivo si lo es"""
    year = date_obj.year
    month = date_obj.month
    day = date_obj.day
...
@st.cache_data
def load_model():
    """Cargar modelo entrenado mejorado"""
    try:
...
# -----------------------------
#   Predicción por clúster - ACTUALIZADA
# -----------------------------
def create_prediction_function(_model_data):
    """Crear función de predicción con todas las features del modelo (orden y shape correctos)."""
    feature_cols = _model_data['feature_cols']  # orden exacto esperado por el modelo

    def predict_risk(cluster_id, hour, temperature, precipitation, wind_speed, date_obj):
        # --- Calendario ---
        day_of_week = date_obj.weekday()                 # 0=Lun..6=Dom
        is_weekend = 1 if day_of_week >= 5 else 0
        dia_festivo = 1 if is_festivo(date_obj) else 0   # <- define is_festivo() fuera
        fiesta = dia_festivo                              # asumimos equivalencia

        # --- Diccionario con TODAS las features (coincidiendo con tu entrenamiento) ---
        fd = {}
        # Meteo directas
        ...
        # Proxy de lluvia por cluster (tu lista de features no incluye 'precipitation (mm)')
        fd['cluster_rain_avg'] = float(precipitation)

        # --- Construir el vector en el ORDEN de feature_cols y con forma (1, n_features) ---
        ...
        pred = int(prob >= _model_data['optimal_threshold'])

        # (Opcional) Ajuste suave por contexto temporal
...
def get_risk_color(probability):
    """Obtener color basado en probabilidad de riesgo"""
    if probability >= 0.10:
        return '#e74c3c'  # Rojo
...
def load_or_build_graph():
    """
    Carga la red vial de Barcelona desde cache (graphml) o la descarga con OSMnx.
    Red simplificada para vehículos (drive).
    """
...
def parse_maxspeed(value):
    """Convierte maxspeed de OSM a km/h."""
    if value is None:
        return None
...
def infer_speed_kmh(edge_data):
    """Estima la velocidad km/h usando maxspeed, o tipo de vía."""
...
def nearest_osm_node(G, lat, lon):
    """Encuentra el nodo de la red más cercano a (lat, lon)."""
    try:
...
EJE_HIGHWAY_TYPES = {"motorway", "trunk", "primary"}  # arterias principales
...
def is_axis_edge(data):
    """True si la arista pertenece al eje vial (por nombre, ref o tipo principal)."""
...
def build_axis_subgraph(G):
    """Crea subgrafo dirigido con SOLO aristas del eje vial."""
    edges_to_keep = []
...
# ====== Riesgo → pesos de ruta ======
def risk_multiplier(prob):
    """Penalización por riesgo: bajo=1, medio≈1.6, alto≈3.0."""
    if prob >= 0.10:   # Alto
        return 3.0
...
def precompute_edge_costs(G, predictions_data, cluster_geometries, buffer_m=200.0, objective="balanced"):
    """
    Para cada arista:
      - Estima velocidad y tiempo base (min).
      - Busca cluster de mayor prob. cercano (<= buffer) y asigna risk_mult.
      - Define pesos de enrutado según 'objective'
    """
    ...
    for cid, geo in cluster_geometries.items():
        if cid in predictions_data:
            clusters.append((cid, geo['lat'], geo['lon'], predictions_data[cid]['probability']))

    buffer_km = buffer_m / 1000.0

    for u, v, k, data in G.edges(keys=True, data=True):
        length_m = float(data.get('length', 0.0))
        length_km = max(0.0001, length_m / 1000.0)
        ...
        best_prob, best_dist = 0.0, 1e9
        for (_, clat, clon, p) in clusters:
            d_km = haversine(y_mid, x_mid, clat, clon)
            if d_km < best_dist:
                best_dist = d_km
                best_prob = p

        mult = risk_multiplier(best_prob) if best_dist <= buffer_km else 1.0

        if objective == "fastest":
            route_weight = time_min
        elif objective == "safest":
            route_weight = time_min * (1.0 + 3.0 * (mult - 1.0))
        else:  # balanced
            route_weight = time_min * (1.0 + 1.5 * (mult - 1.0))
...
def nearest_node_on_graph(G, lat, lon):
    nid = nearest_osm_node(G, lat, lon)
    return nid

def route_between_nodes(G, src, dst):
    """Encuentra ruta con peso 'route_weight' y devuelve coords + totales."""
    try:
...
def compute_route_eje_vial(G_full, predictions_data, cluster_geometries,
    ...
    """
    Ruta coche con estructura: access (G_full) + core (G_axis) + egress (G_full).
    Si core no existe, fallback a G_full.
    """
    if G_full is None:
    ...
    # Precalcular pesos en ambos grafos
    precompute_edge_costs(G_full, predictions_data, cluster_geometries, buffer_m=buffer_m, objective=objective)
    precompute_edge_costs(G_axis, predictions_data, cluster_geometries, buffer_m=buffer_m, objective=objective)
    ...
    # Concatenar coords evitando duplicados en uniones
    def stitch(a, b):
        return a[:-1] + b
...
def create_barcelona_map(predictions_data, cluster_geometries, route_coords=None, origin=None, destination=None):
    """Crear mapa de Barcelona con clusters de riesgo y, opcionalmente, una ruta recomendada."""
    ...
    # Añadir clusters con predicciones
    for cluster_id, prediction_data in predictions_data.items():
        if cluster_id in cluster_geometries:
            ...
            radius_meters = max(50, min(300, probability * 2000))  # 50-300 m
            folium.Circle(
...
def create_heatmap_hour_dow(historical_data):
    """
    Heatmap 24x7 robusto.
    Soporta:
      - {hour -> {dow -> count}}
      - {dow -> {hour -> count}}
    Acepta claves 'str' o 'int' y completa huecos con 0.
    """
    if 'hour_dow_matrix' not in historical_data:
        return None

    mat = historical_data['hour_dow_matrix']
    # Normalizar a {int -> {int -> int}}
    mat_norm = {}
    try:
    ...
    hours = list(range(24))
    dows = list(range(7))  # 0..6
    # ¿Las claves exteriores representan días?
    outer_keys = set(mat_norm.keys())
    is_dow_outer = outer_keys.issubset(set(dows))

    # Construir matriz 24x7 [hora][dow]
    z = []
    if is_dow_outer:
        # mat_norm[dow][hour]
        for h in hours:
    ...
    # Si todo es cero, evita un gráfico “en blanco”
...
def fetch_openmeteo_hourly(lat, lon, start_iso=None, days_ahead=14, past_days=7):
    """
    Descarga horas con temperature_2m, precipitation, wind_speed_10m.
    Devuelve dict con arrays {time[], temp[], precip[], wind[]}
    """
...
def get_weather_for_datetime(target_date: date, hour: int):
    """
    Devuelve (temp_c, precip_mm, wind_kmh, texto_info) para la fecha/hora seleccionadas
    usando el punto del centro de Barcelona.
    """
    ...
    if target_str in times:
        ...
        def parse_t(s):
            try:
                ...
                try:
                    ...
                    return None
        target = parse_t(target_str if ":" in target_str else f"{target_str}:00")
        diffs = []
        for i, s in enumerate(times):
            ...
            if dt is None:
                ...
                diffs.append((abs((dt - target).total_seconds()), i))
        idx = min(diffs)[1]
...
def main():
    ...
    day_info = f"**{day_names[day_of_week]}**"
    
    if is_holiday and holiday_name:
        st.sidebar.info(f"🎉 Festivo: {holiday_name}")
    elif is_weekend:
        st.sidebar.info(f"📅 Fin de semana")
    else:
        st.sidebar.success(f"💼 Día laborable")

    # === METEO desde Open-Meteo ===
    ...
    cm1, cm2, cm3 = st.sidebar.columns(3)
    cm1.metric("Temp", f"{temperature:.1f}°C")
    cm2.metric("Lluvia", f"{precipitation:.1f} mm")
    cm3.metric("Viento", f"{wind_speed:.0f} km/h")
    st.sidebar.caption(f"Centro BCN • {meteo_info}")
    
    # Estadísticas históricas en sidebar
    ...
    if st.sidebar.button("🔮 Actualizar Predicciones", type="primary"):
        ...
        # Guardar los parámetros de predicción actuales
    ...
    if st.session_state.get('update_predictions', False):
        with st.spinner(f"Calculando predicciones para {prediction_date.strftime('%d/%m/%Y')} {prediction_hour:02d}:00..."):
            ...
            for cluster_id in cluster_geometries.keys():
                # Pasar el objeto fecha completo
                ...
                # Recalcular nivel de riesgo después del ajuste
                prediction['risk_level'] = ('Alto' if prediction['probability'] >= 0.10 else 
                                           'Medio' if prediction['probability'] >= 0.05 else 'Bajo')
    ...
    with col2:
        ...
        if st.session_state.predictions_data:
            ...
            for i, (area_id, data) in enumerate(sorted_areas, 1):
                risk_emoji = "🔴" if data['risk_level'] == 'Alto' else "🟠" if data['risk_level'] == 'Medio' else "🟢"
                st.write(f"{risk_emoji} **{i}.** Zona {area_id}: {data['probability']*100:.2f}%")

    # ---------------------------
    # Recomendador de Ruta por EJE VIAL
    # ---------------------------
    ...
    # Captura clics del mapa anterior si existe
    last_click = None
    ...
    # Si hay ruta, re-renderiza el mapa con la ruta + métricas
    if st.session_state.route_result:
        ...
        st.success(
            ...
            f"ETA (modo coche): **{rr['eta_min']:.0f} min**"
        )
    else:
    ...
    with st.expander("ℹ️ Información del Sistema Mejorado"):
        st.markdown(f"""
        ### Acerca del Modelo
        - **Versión**: {model_data['config'].get('model_version', '2.5-enhanced')}
        - **Tipo**: LightGBM con calibración isotonic
        - **Features**: {len(model_data.get('selected_features', []))} características seleccionadas
        - **Datos de entrenamiento**: 2017-2022
        - **Validación**: 2023 | **Test**: 2024
        
        ### Meteorología
        - Fuente: **Open-Meteo** (centro de Barcelona)
        - Los festivos se detectan **automáticamente**
        
        ### Rutas coche (EJE VIAL)
        - Usa la **red vial real** de Barcelona (OSM)
        - Prioriza **Gran Via, Diagonal, Meridiana, Rondas**
        - **Evita zonas de riesgo** penalizando rutas cercanas
        - **ETA realista** basada en velocidades de las vías
        """)
